...
export default function ScanToOrder() {
  ...
  return (
    <section id="scan" className="relative scroll-mt-24 overflow-hidden py-28 lg:py-40">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_20%_20%,rgba(248,183,57,0.12),transparent_58%)]" />
        ...
        <div>
          ...
          <ol className="mt-12 space-y-7">
            {STEPS.map((step, i) => (
              ...
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-saffron-400/35 font-display text-lg text-saffron-300">
                  {i + 1}
                </span>
                <div>
                  <p className="devanagari text-sm text-saffron-400/80">{step.hi}</p>
          ...
          <div className="surface mt-12 p-6">
            ...
            <label className="mt-5 block">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-petrol-950/60 px-4 py-3 text-cream-100 placeholder:text-cream-400/40 transition-colors focus:border-saffron-400 focus:outline-none"
                />
                {table && (
                  <button
                    type="button"
                    onClick={() => setTable('')}
                    className="shrink-0 rounded-xl border border-white/10 px-4 text-[0.65rem] uppercase tracking-widest text-cream-300 transition-colors hover:border-saffron-400/60 hover:text-saffron-300"
                  >
                    Clear
                  </button>
                )}
            ...
            <button
              type="button"
              onClick={copy}
              className="mt-4 flex w-full items-center justify-between gap-3 rounded-xl border border-white/[0.07] bg-petrol-950/50 px-4 py-3 text-left transition-colors hover:border-saffron-400/40"
            >
              <span className="min-w-0 truncate font-mono text-xs text-cream-300/80">{url}</span>
              <span className="shrink-0 text-[0.6rem] uppercase tracking-widest text-saffron-300">
                {copied ? 'Copied' : 'Copy'}
              </span>
            </button>
          </div>
