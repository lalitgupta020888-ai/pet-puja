...
function ThaliCard({ thali }) {
  return (
    <article className="group flex w-[19rem] shrink-0 snap-start flex-col overflow-hidden rounded-3xl border border-white/[0.07] bg-bark-800/60">
      <div className="relative h-40 overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-bark-950 via-bark-950/25 to-transparent" />
        <p className="devanagari absolute bottom-3 left-4 text-lg text-ember-200">{thali.hi}</p>
        <span className="absolute right-3 top-3 rounded-full bg-bark-950/75 px-3 py-1 text-[0.55rem] uppercase tracking-widest text-ember-300 backdrop-blur">
...
function DishCard({ item }) {
  return (
    ...
    <article className="group flex min-w-0 gap-4 rounded-2xl border border-white/[0.06] bg-bark-900/50 p-3 transition-colors duration-300 hover:border-ember-400/25 sm:flex-col sm:gap-0 sm:p-0">
      <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-xl sm:h-44 sm:w-full sm:rounded-b-none sm:rounded-t-2xl">
        ...
        />
        {item.signature && (
          <span className="absolute left-2 top-2 rounded-full bg-bark-950/75 px-2.5 py-1 text-[0.5rem] uppercase tracking-widest text-ember-300 backdrop-blur sm:left-3 sm:top-3">
            Signature
          </span>
        )}
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col sm:p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="shrink-0 font-display text-lg text-ember-300 sm:text-xl">
            ₹{item.price}
          </span>
        </div>
...
export default function OrderClient() {
  ...
  return (
    <div className="min-h-screen pb-32">
      {/* ---------------------------------------------------------- header */}
      <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-bark-950/90 backdrop-blur-xl">
        <div className="shell flex h-16 items-center justify-between gap-4">
          <a href="/" className="group flex items-center gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-ember-400/40 text-ember-300 transition-colors group-hover:bg-ember-400/10">
              <span className="devanagari text-sm">म</span>
            </span>
            <span className="flex flex-col leading-none">
              <span className="devanagari text-[0.6rem] text-ember-400/85">
                {site.nameDevanagari}
              </span>
          ...
          <div className="flex items-center gap-3">
            {table && (
              <span className="inline-flex items-center gap-2 rounded-full border border-ember-400/40 bg-ember-400/10 px-3.5 py-1.5 text-[0.6rem] uppercase tracking-widest text-ember-300">
                Table {table}
              </span>
            )}
      ...
      <section className="relative overflow-hidden border-b border-white/[0.05] py-12 sm:py-16">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(192,88,0,0.14),transparent_60%)]" />

        <div className="shell relative">
          <p className="devanagari text-2xl text-ember-300/85">पूरा मेन्यू</p>
          <h1 className="mt-4 max-w-2xl font-display text-4xl font-light leading-[1.05] tracking-tight text-cream-50 sm:text-5xl">
            The whole kitchen, <span className="italic text-ember-300">on your table</span>
          </h1>
          ...
          <div className="relative mt-8 max-w-md">
            ...
            <input
              ...
              className="w-full rounded-full border border-white/10 bg-bark-900/70 py-3.5 pl-10 pr-4 text-cream-100 placeholder:text-cream-400/50 transition-colors focus:border-ember-400 focus:outline-none"
            />
          </div>
        </div>
      </section>
      ...
      {/* ---------------------------------------------------------- filters */}
      <div className="sticky top-16 z-30 border-b border-white/[0.05] bg-bark-950/90 py-3 backdrop-blur-xl">
      ...
      <main id="menu" className="shell pt-10">
        ...
        {showThalis && (
          <section className="mb-14">
            <div className="flex items-baseline justify-between gap-4">
              <div>
                <p className="devanagari text-lg text-ember-300/85">थाली</p>
        ...
        {grouped.length === 0 ? (
          <div className="py-24 text-center">
            <p className="devanagari text-4xl text-ember-400/25">कुछ नहीं</p>
        ...
        <section className="surface mt-4 p-7">
          <p className="text-[0.62rem] uppercase tracking-brand text-ember-400">
            Before you order
          </p>
