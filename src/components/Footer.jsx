...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-white/[0.07] bg-bark-950 pt-20">
      ...
      <div className="shell">
        <div className="grid gap-12 pb-16 sm:grid-cols-2 lg:grid-cols-4">
          <div className="lg:col-span-2">
            <p className="devanagari text-2xl text-ember-300/85">{site.nameDevanagari}</p>
          ...
          <nav aria-label="Footer">
            <h2 className="text-[0.65rem] uppercase tracking-brand text-ember-400">Explore</h2>
            <ul className="mt-6 space-y-3">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-cream-300/75 transition-colors hover:text-ember-300"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          <div>
            <h2 className="text-[0.65rem] uppercase tracking-brand text-ember-400">Find us</h2>
            <address className="mt-6 space-y-3 not-italic text-cream-300/75">
              <p>{site.addressLine}</p>
              <p>
                <a href={site.phoneHref} className="transition-colors hover:text-ember-300">
                  {site.phone}
                </a>
              </p>
              <p>
              ...
              {site.social.map((s) => (
                <li key={s.label}>
                  <a
                    ...
                    className="flex h-10 items-center rounded-full border border-white/10 px-4 text-[0.62rem] uppercase tracking-widest text-cream-300 transition-colors hover:border-ember-400/60 hover:text-ember-300"
                  >
                    {s.label}
                  </a>
                </li>
