...
const inputClass =
  'w-full rounded-xl border border-white/10 bg-masala-950/60 px-4 py-3 text-cream-100 placeholder:text-cream-400/40 transition-colors focus:border-marigold-400 focus:outline-none';
...
export default function CartDrawer() {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex justify-end bg-masala-950/80 backdrop-blur-sm"
      onClick={closeCart}
    >
      <aside
        role="dialog"
        ...
        className="flex h-full w-full max-w-lg flex-col border-l border-white/[0.08] bg-masala-900 shadow-2xl"
      >
        {/* Head */}
        <header className="flex items-center justify-between border-b border-white/[0.07] px-6 py-5">
          <div>
            <p className="devanagari text-lg text-marigold-300/85">आपका ऑर्डर</p>
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-xl leading-none text-cream-100 transition-colors hover:border-marigold-400 hover:text-marigold-300"
          >
            ×
          </button>
        </header>
        ...
        {step === 'done' && placed && (
          <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full border border-marigold-400/40 text-3xl text-marigold-300">
              ✦
            </div>
            <p className="mt-7 text-[0.65rem] uppercase tracking-brand text-marigold-400">
            ...
            <p className="mt-6 font-display text-2xl text-marigold-300">
        ...
        {step !== 'done' && !count && (
          <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
            <p className="devanagari text-4xl text-marigold-400/25">थाली</p>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5">
              <ul className="space-y-3">
                {items.map((line) => (
                  <li
                    key={line.key}
                    className="flex items-center gap-4 rounded-2xl border border-white/[0.06] bg-masala-950/50 p-3"
                  >
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <span className="font-display text-lg text-marigold-300">
        ...
        {step === 'details' && count > 0 && (
          <form onSubmit={placeOrder} noValidate className="flex flex-1 flex-col overflow-hidden">
            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
              {table && (
                <p className="rounded-xl border border-marigold-400/25 bg-marigold-400/[0.07] px-4 py-3 text-sm leading-relaxed text-cream-200">
                  <span className="devanagari mr-2 text-marigold-300">मेज़</span>
                  You scanned the card on{' '}
                  <strong className="font-semibold text-marigold-300">table {table}</strong> — we
                      ...
                      <button
                        ...
                        className={`rounded-xl border px-3 py-3 text-center transition-all duration-300 ${
                          on
                            ...
                            : 'border-white/10 text-cream-300 hover:border-marigold-400/50'
                        }`}
                      >
                        ...
                        <span
                          className={`devanagari mt-1 block text-[0.7rem] ${
                            on ? 'text-masala-900/70' : 'text-marigold-400/70'
                          }`}
                        >
                          {m.hi}
                        </span>
