  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
        scrolled || open
          ? 'border-b border-white/[0.07] bg-masala-950/85 backdrop-blur-xl'
        ...
        >
          ...
          <nav className="hidden items-center gap-9 lg:flex">
            {navLinks.map((link) => (
              ...
              >
                <span className="devanagari text-[0.6rem] text-marigold-400/0 transition-colors duration-300 group-hover:text-marigold-400/80">
                  {link.hi}
                </span>
                <span className="text-[0.72rem] font-medium uppercase tracking-widest text-cream-300 transition-colors group-hover:text-marigold-200">
                  {link.label}
                </span>
                <span className="absolute -bottom-1.5 h-px w-0 bg-marigold-400 transition-all duration-300 group-hover:w-full" />
              </a>
            ))}
          </nav>
          ...
          <button
            ...
            aria-expanded={open}
            className="flex h-11 w-11 items-center justify-center rounded-full border border-marigold-400/30 text-cream-100 lg:hidden"
          >
        ...
        <nav className="shell flex flex-col py-8">
          ...
          {navLinks.map((link) => (
            <a
              ...
              className="flex items-baseline justify-between border-b border-white/[0.05] py-4 transition-colors hover:text-marigold-200"
            >
              <span className="font-display text-3xl font-light text-cream-100">{link.label}</span>
              <span className="devanagari text-sm text-marigold-400/70">{link.hi}</span>
            </a>
          ))}
