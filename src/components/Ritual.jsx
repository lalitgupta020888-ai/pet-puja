      ...
      <div className="shell">
        ...
        <div className="mt-16 grid gap-px overflow-hidden rounded-3xl border border-white/[0.07] bg-white/[0.05] sm:grid-cols-2 lg:grid-cols-4">
          {ritual.map((step, i) => (
            <div
              key={step.title}
              className="reveal group relative bg-petrol-950 px-7 py-10 transition-colors duration-500 hover:bg-petrol-900"
              style={{ '--reveal-delay': `${i * 90}ms` }}
            >
              <span className="font-display text-sm text-saffron-400/50">
                {String(i + 1).padStart(2, '0')}
              </span>
              <p className="devanagari mt-5 text-2xl text-saffron-300">{step.hi}</p>
              ...
              <span className="absolute inset-x-7 bottom-0 h-px w-0 bg-saffron-400/60 transition-all duration-500 group-hover:w-[calc(100%-3.5rem)]" />
            </div>
          ))}
        </div>
      </div>
